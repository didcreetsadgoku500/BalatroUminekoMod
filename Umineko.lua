  ...
  calculate = function(self, card, context)
    if context.joker_main and card.ability.extra.current_Xmult > 1 then
      return {
        ...
        x_mult_mod = card.ability.extra.current_Xmult,
        colour = G.C.MULT
      }
    end
  end,
  update = function(self, card, dt)
    ...
    if G.playing_cards then
      for _, v in pairs(G.playing_cards) do
        if v.ability.name == "Gold Card" then gold_tally = gold_tally + 1 end
      end
      card.ability.extra.current_Xmult = card.ability.extra.current_Xmult + (gold_tally * card.ability.extra.Xmult_mod)
    end
  end
}